...
const (
	defaultConfigFilename = "tserver.conf"
	defaultDataDirname    = "data"
	defaultLogLevel       = "info"
	defaultLogDirname     = "logs"
	defaultLogFilename    = "tserver.log"
	defaultTrillianHost   = "localhost:8090"

	defaultMainnetPort = "65535"
	defaultTestnetPort = "65534"
)

var (
	defaultHomeDir           = dcrutil.AppDataDir("tserver", false)
	defaultConfigFile        = filepath.Join(defaultHomeDir, defaultConfigFilename)
	defaultDataDir           = filepath.Join(defaultHomeDir, defaultDataDirname)
	defaultSigningKeyFile    = filepath.Join(defaultHomeDir, "signingkey.der")
	defaultEncryptionKeyFile = filepath.Join(defaultHomeDir, "encryption.key")
...
type config struct {
	...
	Version       string
	SigningKey    string `long:"signingkey" description:"File containing the signing private key file"`
	EncryptionKey string `long:"encryptionkey" description:"File containing the encryption private key file"`
...
func loadConfig() (*config, []string, error) {
	...
	cfg := config{
		...
		SigningKey:    defaultSigningKeyFile,
		EncryptionKey: defaultEncryptionKeyFile,
	...
	if preCfg.HomeDir != "" {
		...
		if preCfg.SigningKey == defaultSigningKeyFile {
			cfg.SigningKey = filepath.Join(cfg.HomeDir, "signingkey.der")
		} else {
			cfg.SigningKey = preCfg.SigningKey
		}
		if preCfg.EncryptionKey == defaultEncryptionKeyFile {
			cfg.EncryptionKey = filepath.Join(cfg.HomeDir, "encryption.key")
		} else {
			cfg.EncryptionKey = preCfg.EncryptionKey
		}
