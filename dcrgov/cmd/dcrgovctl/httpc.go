...
// sendReqV3 sends a request to the v1 politeia http API.
func (c *httpc) sendReqV3(method string, route string, reqData interface{}) ([]byte, error) {
	route = fmt.Sprintf("%v%v%v", c.host.String(), v1.APIVersionPrefix, route)
	...
	case http.StatusBadRequest:
		...
		errStr := v1.ErrCodes[e.ErrorCode]
		if e.ErrorContext == "" {
	...
	csrf = r.Header.Get(v1.CSRFTokenHeader)
	if csrf != "" {
		err := c.saveCSRF(csrf)
		if err != nil {
			return nil, err
...
// Versions sends a GET request to the politeia v1 VersionRoute.
func (c *httpc) Version() (*v1.VersionReply, error) {
	b, err := c.sendReqV3(http.MethodGet, v1.VersionRoute, nil)
	if err != nil {
		return nil, err
	}
	var r v1.VersionReply
...
func (c *httpc) Policy() (*v1.PolicyReply, error) {
	b, err := c.sendReqV3(http.MethodGet, v1.PolicyRoute, nil)
	if err != nil {
		return nil, err
	}
	var r v1.PolicyReply
	err = json.Unmarshal(b, &r)
	if err != nil {
		return nil, err
	}
...
func (c *httpc) WriteCmd(cmd v1.Cmd) (*v1.CmdReply, error) {
	b, err := c.sendReqV3(http.MethodPost, v1.WriteRoute, cmd)
	if err != nil {
		return nil, err
	}
	var r v1.CmdReply
...
func (c *httpc) ReadCmd(cmd v1.Cmd) (*v1.CmdReply, error) {
	b, err := c.sendReqV3(http.MethodPost, v1.ReadRoute, cmd)
	if err != nil {
		return nil, err
	}
	var r v1.CmdReply
...
// HTTP request
//
//	URL       : GET https://localhost:4443/v1/version
//	Body      :
