...
func TestNewRecord(t *testing.T) {
	tstoreBackend, cleanup := NewTestTstoreBackend(t)
	defer cleanup()
	...
	for _, test := range recordContentTests {
		t.Run(test.description, func(t *testing.T) {
			// Make backend call
			_, err := tstoreBackend.New(test.metadata, test.files)
...
func TestUpdateUnvettedRecord(t *testing.T) {
	...
	rec, err := tstoreBackend.New(md, fs)
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, test := range recordContentTests {
		t.Run(test.description, func(t *testing.T) {
			...
			// Make backend call
			_, err = tstoreBackend.UpdateUnvettedRecord(token, test.metadata,
	...
	recFrozen, err := tstoreBackend.New(md, fs)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = tstoreBackend.unvetted.treeFreeze(treeIDFromToken(tokenFrozen),
	...
	for _, test := range tests {
		t.Run(test.description, func(t *testing.T) {
			// Make backend call
			_, err = tstoreBackend.UpdateUnvettedRecord(test.token,
...
func TestUpdateVettedRecord(t *testing.T) {
	...
	err = tstoreBackend.unvettedPublish(token, *rec, md, fs)
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, test := range recordContentTests {
		t.Run(test.description, func(t *testing.T) {
			...
			// Make backend call
			_, err = tstoreBackend.UpdateVettedRecord(token, test.metadata,
				[]backend.MetadataStream{}, test.files, test.filesDel)
	...
	err = tstoreBackend.unvettedPublish(tokenFrozen, *recFrozen, md, fs)
	if err != nil {
		t.Fatal(err)
	}
	treeIDFrozenVetted := tstoreBackend.vettedTreeIDs[recFrozen.Token]
	err = tstoreBackend.vetted.treeFreeze(treeIDFrozenVetted,
		backend.RecordMetadata{}, []backend.MetadataStream{}, 0)
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, test := range tests {
		t.Run(test.description, func(t *testing.T) {
			// Make backend call
			_, err = tstoreBackend.UpdateVettedRecord(test.token,
...
func TestUpdateUnvettedMetadata(t *testing.T) {
	...
	for _, test := range recordContentTests {
		t.Run(test.description, func(t *testing.T) {
			// Make backend call
			err := tstoreBackend.UpdateUnvettedMetadata(token,
	...
	for _, test := range tests {
		t.Run(test.description, func(t *testing.T) {
			// Make backend call
			err = tstoreBackend.UpdateUnvettedMetadata(test.token,
...
func TestUpdateVettedMetadata(t *testing.T) {
	...
	for _, test := range recordContentTests {
		t.Run(test.description, func(t *testing.T) {
			// Make backend call
			err := tstoreBackend.UpdateVettedMetadata(token,
				test.metadata, []backend.MetadataStream{})
	...
	for _, test := range tests {
		t.Run(test.description, func(t *testing.T) {
			// Make backend call
			err = tstoreBackend.UpdateVettedMetadata(test.token,
				test.mdAppend, test.mdOverwrite)
...
func TestUnvettedExists(t *testing.T) {
	...
	//
	// Record exists
	result := tstoreBackend.UnvettedExists(token)
	if result == false {
		t.Errorf("got false, want true")
	}
	// Record does not exist
	result = tstoreBackend.UnvettedExists(tokenRandom)
	if result == true {
...
func TestVettedExists(t *testing.T) {
	...
	unvetted, err := tstoreBackend.New(md, fs)
	if err != nil {
		t.Fatal(err)
	}
	...
	vetted, err := tstoreBackend.New(md, fs)
	if err != nil {
		t.Fatal(err)
	}
	...
	md = append(md, newBackendMetadataStream(t, 2, ""))
	err = tstoreBackend.unvettedPublish(tokenVetted, *vetted, md, fs)
	if err != nil {
		t.Fatal(err)
	}
	...
	//
	// Record exists
	result := tstoreBackend.VettedExists(tokenVetted)
	if result == false {
		t.Fatal("got false, want true")
	}
	// Record does not exist
	result = tstoreBackend.VettedExists(tokenUnvetted)
	if result == true {
...
func TestGetUnvetted(t *testing.T) {
	...
	// Bad version error
	_, err = tstoreBackend.GetUnvetted(token, "badversion")
	if err != backend.ErrRecordNotFound {
		t.Errorf("got error %v, want %v", err, backend.ErrRecordNotFound)
	}

	// Bad token error
	_, err = tstoreBackend.GetUnvetted(tokenRandom, "")
	if err != backend.ErrRecordNotFound {
		t.Errorf("got error %v, want %v", err, backend.ErrRecordNotFound)
	}

	// Success
	_, err = tstoreBackend.GetUnvetted(token, "")
	if err != nil {
		t.Errorf("got error %v, want nil", err)
	}
}
...
func TestGetVetted(t *testing.T) {
	...
	// Bad version error
	_, err = tstoreBackend.GetVetted(token, "badversion")
	if err != backend.ErrRecordNotFound {
		t.Errorf("got error %v, want %v", err, backend.ErrRecordNotFound)
	}

	// Bad token error
	_, err = tstoreBackend.GetVetted(tokenRandom, "")
	if err != backend.ErrRecordNotFound {
		t.Errorf("got error %v, want %v", err, backend.ErrRecordNotFound)
	}

	// Success
	_, err = tstoreBackend.GetVetted(token, "")
	if err != nil {
		t.Errorf("got error %v, want nil", err)
	}
}
...
func TestSetUnvettedStatus(t *testing.T) {
	...
	recUnvetToArch, err := tstoreBackend.New(md, fs)
	if err != nil {
		t.Fatal(err)
	}
	...
	recUnvetToUnvet, err := tstoreBackend.New(md, fs)
	if err != nil {
		t.Fatal(err)
	}
	...
	recUnvetToVet, err := tstoreBackend.New(md, fs)
	if err != nil {
		t.Fatal(err)
	}
	...
	recUnvetToCensored, err := tstoreBackend.New(md, fs)
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, test := range tests {
		t.Run(test.description, func(t *testing.T) {
			// Make backend call
			_, err = tstoreBackend.SetUnvettedStatus(test.token, test.status,
				test.mdAppend, test.mdOverwrite)
...
func TestSetVettedStatus(t *testing.T) {
	...
	recVetToUnvet, err := tstoreBackend.New(md, fs)
	if err != nil {
		t.Fatal(err)
	}
	...
	md = append(md, newBackendMetadataStream(t, 2, ""))
	_, err = tstoreBackend.SetUnvettedStatus(tokenVetToUnvet,
		backend.MDStatusVetted, md, []backend.MetadataStream{})
	if err != nil {
		t.Fatal(err)
	}
	// test case: Vetted to vetted
	recVetToVet, err := tstoreBackend.New(md, fs)
	if err != nil {
		t.Fatal(err)
	}
	...
	md = append(md, newBackendMetadataStream(t, 3, ""))
	_, err = tstoreBackend.SetUnvettedStatus(tokenVetToVet,
		backend.MDStatusVetted, md, []backend.MetadataStream{})
	if err != nil {
		t.Fatal(err)
	}
	...
	recVetToArch, err := tstoreBackend.New(md, fs)
	if err != nil {
		t.Fatal(err)
	}
	...
	_, err = tstoreBackend.SetUnvettedStatus(tokenVetToArch,
		backend.MDStatusVetted, md, []backend.MetadataStream{})
	if err != nil {
		t.Fatal(err)
	}
	// test case: Vetted to censored
	recVetToCensored, err := tstoreBackend.New(md, fs)
	if err != nil {
		t.Fatal(err)
	}
	...
	_, err = tstoreBackend.SetUnvettedStatus(tokenVetToCensored,
		backend.MDStatusVetted, md, []backend.MetadataStream{})
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, test := range tests {
		t.Run(test.description, func(t *testing.T) {
			// Make backend call
			_, err = tstoreBackend.SetVettedStatus(test.token, test.status,
				test.mdAppend, test.mdOverwrite)
