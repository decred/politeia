...
func (t *Tstore) TreeFreeze(treeID int64, rm backend.RecordMetadata, metadata []backend.MetadataStream, treePointer int64) error {
...
func (t *Tstore) TreePointer(treeID int64) (int64, bool) {
...
// RecordSave saves the provided record to tstore, creating a new version of
// the record (the record iteration also gets incremented on new versions).
// Once the record contents have been successfully saved to tstore, a
// recordIndex is created for this version of the record and saved to tstore as
// well.
