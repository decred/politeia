...
// saved by the server because all comment add records will be deleted and
// the client will still require those fields to properly display the comment
// hierarchy.
...
func convertCommentDelFromBlobEntry(be store.BlobEntry) (*commentDel, error) {
	// Decode and validate data hint
	b, err := base64.StdEncoding.DecodeString(be.DataHint)
	if err != nil {
		return nil, fmt.Errorf("decode DataHint: %v", err)
...
func convertCommentFromCommentDel(cd commentDel) comments.Comment {
	// Score needs to be filled in seperately
	return comments.Comment{
		Token:     cd.Token,
...
func commentDels(client *tlogbe.PluginClient, merkleHashes [][]byte) ([]commentDel, error) {
	// Retrieve blobs
	blobs, err := client.BlobsByMerkleHash(merkleHashes)
	if err != nil {
		return nil, err
	...
	// Decode blobs
	dels := make([]commentDel, 0, len(blobs))
	for _, v := range blobs {
		be, err := store.Deblob(v)
		if err != nil {
			return nil, err
...
func commentsLatest(client *tlogbe.PluginClient, idx index, commentIDs []uint32) (map[uint32]comments.Comment, error) {
