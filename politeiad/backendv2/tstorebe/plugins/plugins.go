...
type TstoreClient interface {
	...
	CachePut(p PluginClient, blobs map[string][]byte, encrypt bool) error

	// CacheDel deletes the provided blobs from the key-value store. This
	// operation is performed atomically.
	CacheDel(p PluginClient, keys []string) error
	...
	CacheGet(p PluginClient, keys []string) (map[string][]byte, error)
}
