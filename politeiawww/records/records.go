...
func (c *Records) HandleNew(w http.ResponseWriter, r *http.Request) {
	...
	if err := decoder.Decode(&n); err != nil {
		respondWithError(w, r, "HandleNew: unmarshal",
			v1.UserErrorReply{
				ErrorCode: v1.ErrorStatusInputInvalid,
			})
		return
	}
...
func (c *Records) HandleRecords(w http.ResponseWriter, r *http.Request) {
	...
	rsr, err := c.processs(r.Context(), rs, isAdmin)
	if err != nil {
		respondWithError(w, r,
			"HandleRecords: processs: %v", err)
		return
	}
...
func New(cfg *config.Config, pdc *pdclient.Client, s *sessions.Sessions, e *events.Manager) *Comments {
	return &Comments{
		cfg:       cfg,
		politeiad: pdc,
		sessions:  s,
