...
// cmdTestRun performs a test run of cmswww routes.
...
// invoiceNewTest submits a new invoice for the provided user. The invoice is
...
func invoiceNewTest(u user, contractorRate uint, labor float64, pc pclient.Client) (*rcv1.CensorshipRecord, error) {
	err := login(u)
	if err != nil {
		return nil, fmt.Errorf("login: %v", err)
	}
	...
	return &nr.Record.CensorshipRecord, nil
}
...
func testDCCVote(admin user) error {
	...
	cr1, err := invoiceNewTest(*c1, 40, 50, *pc)
	if err != nil {
		return err
	}
	cr2, err := invoiceNewTest(*c2, 40, 100, *pc)
	if err != nil {
		return err
	}
	cr3, err := invoiceNewTest(*c3, 40, 150, *pc)
	if err != nil {
		return err
	}
